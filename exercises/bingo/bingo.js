function bingo() {
  ...
  function newRound() {
      ...
      if (askContinue() && !getBingo()) {
        ...
        for (let i = 0; i < card.length; i++) {
          if (ball === card[i].number) {
            card[i].matched = true;
            if (!line && getLine()) {
              alert("¡Tienes el número " + ball + ", has completado una linea!");
              line = true;
            } else {
    ...
    alert("¡Bingo!\nHas requerido " + balls.length + " rondas y has conseguido una puntuación de " + scoreCount(balls.length) + ".");
    mainRanking();
  ...
  function newBingoBall() {
    ...
    alert(
      "El siguiente número es... el " + newBall + "!"
    );
    balls.push(newBall);
    return newBall;
  }
