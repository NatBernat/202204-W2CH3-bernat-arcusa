const pasapalabraGame = () => {
    ...
    const coreGame = () => {
        ...
        alert(`${playerName}, en este juego debes acertar una palabra que empiece o contenga cada una de las letras del abecedario en español.\n` +
            ...
            for (let i = 0; i < questions.length; i++) {
                if (questions[i].status === 0 && !exitGame) {
                    ...
                    if (userAnswer.toLowerCase() === questions[i].answer) {
                        ...
                        alert(`¡Has fallado, pierdes un punto! La respuesta correcta era ${questions[i].answer}.`)
                        questions[i].status = -1;
                    };
                };
            };
        } while (!completenessCheck() && !exitGame) {
            if (exitGame) {alert(`Has finalizado la partida con ${checkPoints()} puntos.`)};
        };
    ...
    const getPlayerName = () => {
        const playerName = prompt("¡Bienvenido al juego del Pasapalabra! ¿Cual es tu nombre?");
